...
public class DbUtils {
    ...
    public static class Deserialize {
        ...
        public static Optional<Location<World>> location ( Document doc ) {
            ...
            Optional<World> worldOpt = Sponge.getServer().getWorld( UUID.fromString( doc.getString("world") ) );
            return worldOpt.map(world1 -> new Location<>(world1, position));
        }

        public static Optional<PlotDefinition> definition( Document doc ) {
            Optional<World> world = Sponge.getServer().getWorld( UUID.fromString( doc.getString("world") ) );
            if ( world.isPresent() ) {
                ...
                AtherysTowns.getInstance().getLogger().error("[MongoDB] Failed to deserialize plot definition.");
