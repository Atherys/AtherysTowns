...
@Aliases({"create", "new"})
@Description("Used to create a new town. If you are already part of a town, you must leave your current town first.")
@Permission("atherystowns.town.create")
public class TownCreateCommand extends TownsCommand implements ParameterizedCommand {
    ...
    protected CommandResult execute(Player player, CommandContext args, Resident resident,
        ...
        if (resident.getTown().isPresent()) {
            TownMessage.warn(player, Text.of(
                    "You are already part of a town. Please leave your current town first in order to create a new one."));
        ...
        try {
            ...
            TownMessage.warn(player, Text.of(
                    "Your plot definition did not meet the requirements. You may clear your selection with ",
                    TextStyles.BOLD, " /p desel", TextStyles.RESET, TextColors.RED,
                    " and try creating a town from the chunk you are standing in."));
            return CommandResult.empty();
            ...
            if (!chunk.isPresent()) {
                TownMessage.warn(player,
                        "Plugin could not find chunk. You must therefore create a plot definition first in order to create the town. This will become the Home plot of your town, and the location where you are standing ( inside of the definition ) will become the spawn point of the town.");
                return CommandResult.empty();
            } else {
                try {
        ...
        if (define.isPresent()) {
            ...
            Town t = Town.create(define.get(), resident,
                    args.<String>getOne(Text.of("townName")).orElse(player.getName() + "'s Town"),
                    AtherysTowns.getConfig().TOWN.INITIAL_AREA);
            if (n.isPresent()) {
                t.setParent(n.get());
            } else {
                TownMessage.warn(player,
                        "The nation you specified was invalid. Town was not added to a nation.");
            }
