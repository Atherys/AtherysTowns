...
@Aliases("deposit")
@Description("Used to deposit currency into the town bank.")
@Permission("atherystowns.town.deposit")
public class TownDepositCommand extends TownsCommand implements ParameterizedCommand {
            ...
            if (town.getBank().isPresent()) {
                ...
                if (result) {
                    town.informResidents(
                            Text.of(player.getName(), " has deposited ", amount.toString(), " ",
                                    currency.getPluralDisplayName(), " into the town bank."));
                } else {
                    TownMessage.warn(player, "Deposit Failed.");
                }
