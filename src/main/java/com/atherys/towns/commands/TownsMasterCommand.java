...
public abstract class TownsMasterCommand extends TownsSimpleCommand {
    ...
    public void showHelp(String cmd, Player player) {
        ...
        player.sendMessage(
            Text.of(decoration, ".o0o.=---------= { ", TextStyles.BOLD, primary, "/", cmd, " Help",
                TextStyles.RESET, decoration, " } =---------=.o0o."));
        for (Map.Entry<List<String>, CommandCallable> entry : getChildren().entrySet()) {
            Text cmdHelp = Text
                .of(TextStyles.BOLD, primary, "/", cmd, " ", entry.getKey().get(0), " ",
                    entry.getValue().getUsage(player));
