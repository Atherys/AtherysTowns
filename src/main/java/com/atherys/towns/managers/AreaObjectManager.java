 ...
 * mind. This class utilizes caching.
 *
...
public abstract class AreaObjectManager<T extends AreaObject> extends
    ...
    /**
     * Retrieve all {@link AreaObject}s from the database whose name equals ( See: {@link
     * String#equals(Object)} ) the given String.
     *
    ...
    /**
     * Uses {@link #getByName(String)} to retrieve a list of results and instead returns the first
     * result within the list. If the returned list is empty, this will return an empty optional.
     *
     * @param name The name to check for
     * @return The result
     */
     ...
     * @return An optional containing the result. If no object could be found given these parameters,
     * this will be empty.
     */
    ...
    /**
     * Retrieve an {@link AreaObject} based on whether or not the provided location is contained
     * within it. AreaObjects must not be permitted to overlap.
     *
    ...
    /**
     * Add a new {@link AreaObject} to the cache. This will be saved to the database at an appropriate
     * time.
     *
     * @param ao The AreaObject to be cached.
     */
    public void add(T ao) {
        if (super.getCache().containsKey(ao.getUUID())) {
            return;
        }
