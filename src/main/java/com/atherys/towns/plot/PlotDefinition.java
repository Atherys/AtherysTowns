...
public class PlotDefinition extends Rectangle2D {
    ...
    public static Optional<PlotDefinition> fromPlayer(Player player, Town town) throws DefinitionNotValidException, DefinitionNotPresentException {
        return Optional.of( new PlotDefinition(  player.getWorld(), checkPlayer(player, town) ) );
    }

    public static Optional<PlotDefinition> fromChunk ( Player player, Town town, Chunk chunk ) throws DefinitionNotValidException, DefinitionNotPresentException {
    ...
    public PlotDefinition shrink ( double x, double y, double width, double height ) {
        return new PlotDefinition( world, super.x0 + x, super.y0 + y, super.w - 2*width, super.h - 2*height );
    }
    ...
    public boolean isBordering ( PlotDefinition definition ) {
        if ( !this.world.equals(definition.world) ) return false;
    ...
    private static Rectangle2D checkPlayer (Player player, Town town, Vector3d pos1, Vector3d pos2 ) throws DefinitionNotValidException, DefinitionNotPresentException {
        ...
        if ( AtherysTowns.getInstance().getPlotManager().checkIntersection( new PlotDefinition( player.getWorld(), test ) ) ) {
        ...
        if ( !test.contains( player.getLocation().getPosition().getX(),  player.getLocation().getPosition().getZ() ) ) {
    ...
    private static Rectangle2D checkPlayer ( Player player, Town town ) throws DefinitionNotValidException, DefinitionNotPresentException {
        Optional<Object> loc1Opt = TownsValues.get(player.getUniqueId(), TownsValues.TownsKey.PLOT_SELECTOR_1ST);
        Optional<Object> loc2Opt = TownsValues.get(player.getUniqueId(), TownsValues.TownsKey.PLOT_SELECTOR_2ND);

        if (!loc1Opt.isPresent() || !loc2Opt.isPresent()) {
            TownMessage.warn(player, Text.of("You must use the plot select tool ( /p tool ), right-click at your first location and left-click and your second location in order to set up a plot definition."));
        ...
        if ( !loc1.getExtent().equals(loc2.getExtent()) ) {
            TownMessage.warn(player, Text.of("Corners of definition must be within the same world!"));
            throw new DefinitionNotValidException();
        }
